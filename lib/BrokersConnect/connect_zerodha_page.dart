...
import 'package:optionxi/BrokersPage/Zerodha/homepage_zerodha.dart';
...
class _ZerodhaConnectPageState extends State<ZerodhaConnectPage>
  ...
  void initState() {
    super.initState();
    developer.log('🚀 [ZerodhaConnectPage] Initializing page...',
        name: 'ZerodhaConnect');
  ...
  @override
  void dispose() {
    developer.log('🧹 [ZerodhaConnectPage] Disposing resources...',
        name: 'ZerodhaConnect');
  ...
  Future<void> _initializePage() async {
    developer.log('🔧 [ZerodhaConnectPage] Starting page initialization...',
        name: 'ZerodhaConnect');
    ...
    if (user != null) {
      ...
      developer.log('❌ [ZerodhaConnectPage] No authenticated user found',
          name: 'ZerodhaConnect');
      if (mounted) {
        setState(() {
    ...
    developer.log('✅ [ZerodhaConnectPage] Page initialization complete',
        name: 'ZerodhaConnect');
  }

  void _setupRealtimeListener() {
    if (_suid == null) return;
    developer.log('🔄 [ZerodhaConnectPage] Setting up real-time listener...',
        name: 'ZerodhaConnect ');
    _realtimeListener = _dbRef.onValue.listen(
      (DatabaseEvent event) {
        developer.log('📡 [ZerodhaConnectPage] Real-time update received',
            name: 'ZerodhaConnect');
        if (mounted) _handleRealtimeUpdate(event);
      },
      onError: (error) {
        developer.log('❌ [ZerodhaConnectPage] Real-time listener error: $error',
            error: error, name: 'ZerodhaConnect');
  ...
  Future<void> _loadConnectionDetails() async {
    ...
    try {
      ...
      if (snapshot.exists && snapshot.value != null) {
        developer.log('📊 [ZerodhaConnectPage] Connection details found',
            name: 'ZerodhaConnect');
        final data = Map<String, dynamic>.from(snapshot.value as Map);
        _updateStateFromData(data, isInitialLoad: true);
      } else {
        developer.log('📭 [ZerodhaConnectPage] No connection details found',
            name: 'ZerodhaConnect');
        if (mounted) {
          setState(() {
            _status = 'disconnected';
      ...
      developer.log('❌ [ZerodhaConnectPage] Error loading details: $e',
          error: e, name: 'ZerodhaConnect');
      if (mounted) {
        setState(() {
          _status = 'error';
  ...
  void _updateStateFromData(Map<String, dynamic> data,
    ...
    switch (newStatus) {
      ...
      case 'credentials_saved':
        newStatusMessage =
            'Credentials saved. Please authenticate with Kite to proceed.';
        break;
      case 'pending':
    ...
    if (_status != newStatus ||
      ...
      developer.log(
          '📊 [ZerodhaConnectPage] State updated - Status: $newStatus',
          name: 'ZerodhaConnect');
    }
  }
  ...
  /// NEW FLOW: Step 1 - Save API Key and Secret
  Future<void> _saveCredentials() async {
    if (!_formKey.currentState!.validate()) {
      developer.log('❌ [ZerodhaConnectPage] Form validation failed',
          name: 'ZerodhaConnect');
      return;
    }

    developer.log('🔄 [ZerodhaConnectPage] Saving credentials...',
        name: 'ZerodhaConnect');
    ...
    try {
      ...
      developer.log('✅ [ZerodhaConnectPage] Credentials saved successfully',
          name: 'ZerodhaConnect');
      _showStatusNotification(
          'Credentials Saved',
          'Next, authenticate with Kite to complete the connection.',
          Colors.blue,
          Icons.save_rounded);
    } catch (e) {
      developer.log('❌ [ZerodhaConnectPage] Save failed: $e',
          error: e, name: 'ZerodhaConnect');
  ...
  /// NEW FLOW: Step 2 - Launch Kite authentication URL
  Future<void> _launchKiteAuthentication() async {
    if (_apiKeyController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please save a valid API Key first.')));
      return;
    }
    developer.log('🎥 [ZerodhaConnectPage] Launching Kite authentication...',
        name: 'ZerodhaConnect');
    final suid = FirebaseAuth.instance.currentUser!.uid.toString();
    final url = Uri.parse(
        'https://kite.trade/connect/login?v=3&api_key=${_apiKeyController.text.trim()}&redirect_params=suid%3D$suid');
    print(url);
    if (await canLaunchUrl(url)) {
      await launchUrl(url, mode: LaunchMode.externalApplication);
    } else {
      developer.log('❌ [ZerodhaConnectPage] Failed to launch Kite URL',
          name: 'ZerodhaConnect');
  ...
  Future<void> _disconnectAccount() async {
    ...
    developer.log('🔌 [ZerodhaConnectPage] Disconnect account requested',
        name: 'ZerodhaConnect');
    ...
    if (confirm == true) {
      ...
      developer.log('✅ [ZerodhaConnectPage] Account disconnected successfully',
          name: 'ZerodhaConnect');
    }
  }
    ...
    switch (_status) {
      ...
      case 'connection_failed':
        return Column(
          ...
          children: [
            FilledButton.icon(
              onPressed: _launchKiteAuthentication,
              icon: const Icon(Icons.security_rounded),
              label: const Text('Authenticate with Kite'),
              style: FilledButton.styleFrom(
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Container(
                    ...
                    child: ClipRRect(
                      ...
                      child: Image.asset(
                        "assets/brokers/kite.png", // e.g., "assets/image/broker.png"
                        width: 50,
                        height: 50,
                        fit: BoxFit
              ...
              Text(
                'Your Zerodha account is connected and ready for trading.',
    ...
    final button = FilledButton.icon(
      onPressed: () async {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => HomepageZerodha(),
          ),
        );
      },
  ...
  Widget _buildRedirectNote(ThemeData theme) {
    const String redirectUrl = 'https://fastapi.optionxi.com/broker/zerodha';
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              'You should add the redirect url in your zerodha developer app. You can use the FREE Personal app type',
              style: theme.textTheme.bodyMedium?.copyWith(
                  height: 1.6,
                  color:
            ...
            GestureDetector(
              onTap: () async {
                ...
                _showStatusNotification(
                    'Link Copied',
                    'Paste the link as the redirect url in the zerodha developer portal',
                    Colors.blue,
                    Icons.link);
              },
              child: Container(
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text('go to zerodha developer page'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
                'Follow our video guide to get your API credentials from Zerodha.'),
            const SizedBox(height: 16),
            FilledButton.icon(
