...
import 'package:optionxi/BrokersPage/Upstox/homepage_upstox.dart';
...
class _UpstoxConnectPageState extends State<UpstoxConnectPage>
  ...
  void initState() {
    super.initState();
    developer.log('🚀 [UpstoxConnectPage] Initializing page...',
        name: 'UpstoxConnect');
  ...
  @override
  void dispose() {
    developer.log('🧹 [UpstoxConnectPage] Disposing resources...',
        name: 'UpstoxConnect');
  ...
  Future<void> _initializePage() async {
    developer.log('🔧 [UpstoxConnectPage] Starting page initialization...',
        name: 'UpstoxConnect');
    ...
    if (user != null) {
      ...
      developer.log('❌ [UpstoxConnectPage] No authenticated user found',
          name: 'UpstoxConnect');
      if (mounted) {
        setState(() {
    ...
    developer.log('✅ [UpstoxConnectPage] Page initialization complete',
        name: 'UpstoxConnect');
  }

  void _setupRealtimeListener() {
    if (_suid == null) return;
    developer.log('🔄 [UpstoxConnectPage] Setting up real-time listener...',
        name: 'UpstoxConnect');
    _realtimeListener = _dbRef.onValue.listen(
      (DatabaseEvent event) {
        developer.log('📡 [UpstoxConnectPage] Real-time update received',
            name: 'UpstoxConnect');
  ...
  void _handleRealtimeUpdate(DatabaseEvent event) {
    ...
    if (snapshot.exists && snapshot.value != null) {
      ...
      if (_status != 'disconnected') {
        ...
        developer.log(
            '📭 [UpstoxConnectPage] Real-time update: Data removed, status reset',
            name: 'UpstoxConnect');
      }
    }
  }
  ...
  Future<void> _loadConnectionDetails() async {
    if (_suid == null) return;
    developer.log(
        '📥 [UpstoxConnectPage] Loading connection details from Firebase...',
        name: 'UpstoxConnect');
    try {
      final snapshot = await _dbRef.get();
      if (snapshot.exists && snapshot.value != null) {
        developer.log('📊 [UpstoxConnectPage] Connection details found',
            name: 'UpstoxConnect');
        final data = Map<String, dynamic>.from(snapshot.value as Map);
        _updateStateFromData(data, isInitialLoad: true);
      } else {
        developer.log('📭 [UpstoxConnectPage] No connection details found',
            name: 'UpstoxConnect');
        if (mounted) {
          setState(() {
            _status = 'disconnected';
      ...
      developer.log('❌ [UpstoxConnectPage] Error loading details: $e',
          error: e, name: 'UpstoxConnect');
      if (mounted) {
        setState(() {
          _status = 'error';
  ...
  void _updateStateFromData(Map<String, dynamic> data,
    ...
    switch (newStatus) {
      case 'authenticated':
        newStatusMessage = 'You are connected to Upstox and ready to trade!';
      ...
      case 'credentials_saved':
        newStatusMessage =
            'Credentials saved. Please authenticate with Upstox to proceed.';
        break;
      case 'pending':
    ...
    if (_status != newStatus ||
      ...
      developer.log('📊 [UpstoxConnectPage] State updated - Status: $newStatus',
          name: 'UpstoxConnect');
    }
  }
  ...
  Future<void> _saveCredentials() async {
    if (!_formKey.currentState!.validate()) {
      developer.log('❌ [UpstoxConnectPage] Form validation failed',
          name: 'UpstoxConnect');
      return;
    }

    developer.log('🔄 [UpstoxConnectPage] Saving credentials...',
        name: 'UpstoxConnect');
    ...
    try {
      ...
      developer.log('✅ [UpstoxConnectPage] Credentials saved successfully',
          name: 'UpstoxConnect');
      _showStatusNotification(
          'Credentials Saved',
          'Next, authenticate with Upstox to complete the connection.',
          Colors.blue,
          Icons.save_rounded);
    } catch (e) {
      developer.log('❌ [UpstoxConnectPage] Save failed: $e',
          error: e, name: 'UpstoxConnect');
  ...
  Future<void> _launchUpstoxAuthentication() async {
    ...
    developer.log('🎥 [UpstoxConnectPage] Launching Upstox authentication...',
        name: 'UpstoxConnect');
    final suid = FirebaseAuth.instance.currentUser!.uid.toString();
    final url = Uri.parse(
        'https://api.upstox.com/v2/login/authorization/dialog?response_type=code&client_id=${_apiKeyController.text.trim()}&redirect_uri=https://fastapi.optionxi.com/broker/upstox&response_type=code&state=$suid');
    print(url);
    if (await canLaunchUrl(url)) {
      await launchUrl(url, mode: LaunchMode.externalApplication);
    } else {
      developer.log('❌ [UpstoxConnectPage] Failed to launch Upstox URL',
          name: 'UpstoxConnect');
  ...
  Future<void> _disconnectAccount() async {
    ...
    developer.log('🔌 [UpstoxConnectPage] Disconnect account requested',
        name: 'UpstoxConnect');
    ...
    if (confirm == true) {
      ...
      developer.log('✅ [UpstoxConnectPage] Account disconnected successfully',
          name: 'UpstoxConnect');
    }
  }
    ...
    switch (_status) {
      ...
      case 'connection_failed':
        return Column(
          ...
          children: [
            FilledButton.icon(
              onPressed: _launchUpstoxAuthentication,
              icon: const Icon(Icons.security_rounded),
              label: const Text('Authenticate with Upstox'),
              style: FilledButton.styleFrom(
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Container(
                    ...
                    child: ClipRRect(
                      ...
                      child: Image.asset(
                        "assets/brokers/upstox.png", // e.g., "assets/image/broker.png"
                        width: 50,
                        height: 50,
                        fit: BoxFit
              ...
              Text(
                'Your Upstox account is connected and ready for trading.',
  ...
  Widget _buildLoadingScreen(ThemeData theme, String text) {
    return Scaffold(
        appBar: AppBar(title: const Text('Connect to Upstox')),
        body: Center(
            child:
  ...
  Widget _buildRedirectNote(ThemeData theme) {
    const String redirectUrl = 'https://fastapi.optionxi.com/broker/upstox';
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              'You should add the redirect url in your upstox developer app',
              style: theme.textTheme.bodyMedium?.copyWith(
                  height: 1.6,
                  color:
            ...
            GestureDetector(
              onTap: () async {
                ...
                _showStatusNotification(
                    'Link Copied',
                    'Paste the link as the redirect url in the upstox developer portal',
                    Colors.blue,
                    Icons.link);
              },
              child: Container(
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text('go to upstox developer page'),
  ...
  Widget _buildHelpSection(ThemeData theme) {
    void launchYouTube() async {
      final url = Uri.parse('https://www.youtube.com/watch?v=XZH3j2P07mo');
      if (await canLaunchUrl(url)) {
        await launchUrl(url, mode: LaunchMode.externalApplication);
      }
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
                'Follow our video guide to get your API credentials from Upstox.'),
            const SizedBox(height: 16),
            FilledButton.icon(
