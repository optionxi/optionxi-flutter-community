...
// --- Main Widget ---
class AngelOneConnectPage extends StatefulWidget {
  const AngelOneConnectPage({Key? key}) : super(key: key);

  @override
  State<AngelOneConnectPage> createState() => _AngelOneConnectPageState();
}

class _AngelOneConnectPageState extends State<AngelOneConnectPage>
  ...
  void initState() {
    super.initState();
    print('🚀 [AngelOneConnectPage] Initializing page...');
  ...
  Future<void> _initializePage() async {
    print('🔧 [AngelOneConnectPage] Starting page initialization...');

    User? user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      print('👤 [FyeresConnectPage] User authenticated: ${user.uid}');
      _suid = user.uid;
      _dbRef = FirebaseDatabase.instance.ref().child('brokers/angelone/$_suid');

      // Load initial data and setup real-time listener
      await _loadConnectionDetails();
      _setupRealtimeListener();
    } else {
      print('❌ [AngelOneConnectPage] No authenticated user found');
      setState(() {
    ...
    print('✅ [AngelOneConnectPage] Page initialization complete');
  }
  ...
  void _setupRealtimeListener() {
    ...
    print('🔄 [AngelOneConnectPage] Setting up real-time listener...');

    _realtimeListener = _dbRef.onValue.listen(
      (DatabaseEvent event) {
        print('📡 [AngelOneConnectPage] Real-time update received');
        _handleRealtimeUpdate(event);
      },
      onError: (error) {
        print('❌ [AngelOneConnectPage] Real-time listener error: $error');
  ...
  void _handleRealtimeUpdate(DatabaseEvent event) {
    ...
    if (snapshot.exists && snapshot.value != null) {
      ...
      if (_status != newStatus ||
        ...
        print(
            '📊 [AngelOneConnectPage] Real-time update applied - Status: $newStatus');
      }
    } else {
      ...
      if (_status != 'disconnected') {
        ...
        _pulseController.stop();
        print(
            '📭 [AngelOneConnectPage] Real-time update: Data removed, status reset');
      }
    }
  }
  ...
  Future<void> _loadConnectionDetails() async {
    if (_suid == null) {
      print(
          '❌ [AngelOneConnectPage] Cannot load connection details: SUID is null');
      setState(() {
        _status = 'disconnected';
        _statusMessage = 'Please sign in to connect your broker account.';
      });
    ...
    print(
        '📥 [AngelOneConnectPage] Loading connection details from Firebase...');
    setState(() {
      _status = 'checking';
    ...
    try {
      ...
      if (snapshot.exists && snapshot.value != null) {
        print('📊 [AngelOneConnectPage] Connection details found in Firebase');
        final data = Map<String, dynamic>.from(snapshot.value as Map);
        ...
        print(
            '📭 [AngelOneConnectPage] No connection details found in Firebase');
        setState(() {
          _status = 'disconnected';
          _statusMessage = 'Enter credentials to connect.';
        });
      ...
      print('❌ [AngelOneConnectPage] Error loading connection details: $e');
      setState(() {
        _status = 'error';
  ...
  @override
  void dispose() {
    print('🧹 [AngelOneConnectPage] Disposing resources...');
  ...
  Future<void> _saveAndConnectCredentials() async {
    if (!_formKey.currentState!.validate()) {
      print('❌ [AngelOneConnectPage] Form validation failed');
      return;
    }

    print('🔄 [AngelOneConnectPage] Starting credential save process...');
    setState(() {
      _isLoading = true;
      _status = 'pending';
    ...
    try {
      ...
      print(
          '✅ [AngelOneConnectPage] Credentials saved successfully to Firebase');

      // Note: State will be updated automatically by real-time listener
      print('🎉 [AngelOneConnectPage] Save process completed successfully!');
    } catch (e) {
      print('❌ [AngelOneConnectPage] Save failed: $e');
      ...
      try {
        ...
        print('💾 [AngelOneConnectPage] Firebase updated with error status');
      } catch (dbError) {
        print(
            '❌ [AngelOneConnectPage] Failed to update Firebase with error: $dbError');
      }
      ...
      print('🔄 [AngelOneConnectPage] Save process finished');
    }
  }

  Future<void> _disconnectAccount() async {
    print('🔌 [AngelOneConnectPage] Disconnect account requested');

    print(
        '🗑️ [AngelOneConnectPage] User confirmed disconnect, removing data...');
    await _dbRef.remove();
    ...
    print('✅ [AngelOneConnectPage] Account disconnected successfully');
  }

  void _launchYouTube() async {
    print('🎥 [AngelOneConnectPage] Launching YouTube guide...');
    final url = Uri.parse('https://www.youtube.com/watch?v=z7swHkB3Pa0');
    if (await canLaunchUrl(url)) {
      await launchUrl(url, mode: LaunchMode.externalApplication);
      print('✅ [AngelOneConnectPage] YouTube guide launched successfully');
    } else {
      print('❌ [AngelOneConnectPage] Failed to launch YouTube guide');
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    if (_isAuthChecking) {
      return Scaffold(
        appBar: AppBar(
          title: Row(
            children: [
              const Text('Connect to AngelOne'),
                                  ...
                                  content: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        ...
                                        '• Disconnect your AngelOne account\n'
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'Follow this video guide to get your API credentials from the AngelOne Developer portal.',
              style: theme.textTheme.bodyMedium?.copyWith(
                height: 1.6,
