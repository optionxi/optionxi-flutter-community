...
// ─── Design System (Light-first, minimal) ────────────────────────────────────
class DS {
  // Core backgrounds
  ...
  // Text
  static const textPrimary = Color(0xFF0D1421);
  static const textSecondary = Color(0xFF5A6480);
  static const textTertiary = Color(0xFF9AA3BB);

  // Semantic colours
  static const green = Color(0xFF00A86B); // Bullish
  static const red = Color(0xFFE5303F); // Bearish
  static const amber = Color(0xFFE8920D); // Warning / neutral
  static const blue = Color(0xFF2563EB); // Primary action
  static const violet = Color(0xFF7C3AED); // ATLAS
  static const teal = Color(0xFF0891B2); // Bollinger
  static const orange = Color(0xFFEA580C); // Chartink
...
class _NiftyAiSummaryPageState extends State<NiftyAiSummaryPage>
        ...
        child: Column(children: [
              ...
              builder: (_, __) => Stack(alignment: Alignment.center, children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(color: DS.blue.withOpacity(.08), blurRadius: 16)
                    ],
                  ),
          ...
          FadeTransition(
              ...
              child: Column(children: [
                ...
                Text(step.sub,
                    textAlign: TextAlign.center,
                    style: GoogleFonts.dmSans(
                        color: DS.textSecondary, fontSize: 13, height: 1.5)),
              ])),
          const SizedBox(height: 28),
          // Progress
          ClipRRect(
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('${_stepIndex + 1} of ${_kSteps.length}',
                style: GoogleFonts.jetBrainsMono(
                    color: DS.textTertiary, fontSize: 10)),
            Text('${(pct * 100).round()}%',
                style: GoogleFonts.jetBrainsMono(
                    color: DS.blue, fontSize: 10, fontWeight: FontWeight.w600)),
          ]),
                        ...
                        child: Text(_kSteps[i].label,
                            style: GoogleFonts.dmSans(
                                ...
                                fontWeight: state == 1
                                    ...
                                    : FontWeight.w400))),
                    if (state == 0)
                      const Icon(Icons.check_rounded, color: DS.green, size: 15)
                    else if (state == 1)
                      SizedBox(
                          width: 15,
                          height: 15,
                      ...
                      Container(
                          ...
                          decoration: const BoxDecoration(
                              shape: BoxShape.circle, color: DS.border)),
                  ]),
                ),
                if (i < _kSteps.length - 1)
                  const Divider(
                      height: 1, indent: 58, endIndent: 16, color: DS.divider),
              ]);
            })),
          ),
          const SizedBox(height: 32),
        ...
        Expanded(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Check your connection and try again.',
                style:
                    GoogleFonts.dmSans(color: DS.textSecondary, fontSize: 14)),
          ...
          slivers: [
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                ...
                // 1. Signal Verdict (hero)
                _SignalVerdictCard(verdict: s.signalVerdict),
                const SizedBox(height: 14),

                // 2. Nifty price + AI one-liner
                _NiftyPriceCard(summary: s),
                const SizedBox(height: 20),

                // 4. Nifty 5-min chart
                _label('Nifty 5-Min Chart', Icons.candlestick_chart_rounded,
                    DS.blue),
                const SizedBox(height: 8),
                ...
                // 5. VIX Panel
                _label('India VIX', Icons.shield_rounded, DS.violet),
                const SizedBox(height: 8),
                _VixCard(summary: s),
                const SizedBox(height: 20),

                // 6. ATLAS
                _label('ATLAS Indicators', Icons.hub_rounded, DS.violet),
                const SizedBox(height: 8),
                ...
                // 7. Bollinger
                _label(
                    'Bollinger Breakouts', Icons.bubble_chart_rounded, DS.teal),
                const SizedBox(height: 8),
                ...
                // 8. Chartink
                _label(
                    'Chartink Probability', Icons.bar_chart_rounded, DS.orange),
                const SizedBox(height: 8),
                ...
                // 9. Trade Setup
                _label(
                    'Trade Setup', Icons.arrow_circle_right_rounded, DS.green),
                const SizedBox(height: 8),
                _TradeCard(idea: s.tradeIdea),
                const SizedBox(height: 20),

                // 10. Key Levels
                _label('Key Levels', Icons.layers_rounded, DS.textSecondary),
                const SizedBox(height: 8),
                _KeyLevelsCard(support: s.support, resistance: s.resistance),
                const SizedBox(height: 20),

                // 11. Live Indices
                _label('Live Watchlist', Icons.show_chart_rounded,
                    DS.textSecondary),
                const SizedBox(height: 8),
                ...
                // 12. Key Observations
                _label('Key Observations', Icons.lightbulb_outline_rounded,
                    DS.amber),
                const SizedBox(height: 8),
                ...
                if (s.riskFactors.isNotEmpty) ...[
                  _label('Risk Factors', Icons.warning_amber_rounded, DS.red),
                  const SizedBox(height: 8),
  ...
  Widget _label(String title, IconData icon, Color color) => Row(children: [
...
class _SignalVerdictCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool atlasOk = verdict['atlas_actionable'] as bool? ?? false;
    final bool bollOk = verdict['bollinger_actionable'] as bool? ?? false;
    final bool chartOk = verdict['chartink_actionable'] as bool? ?? false;
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: cc.withOpacity(.06), blurRadius: 20, spreadRadius: 2),
          BoxShadow(
              color: Colors.black.withOpacity(.04),
              blurRadius: 10,
              offset: const Offset(0, 4)),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
            decoration: BoxDecoration(
              color: DS.sentiment(bias).withOpacity(.1),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: DS.sentiment(bias).withOpacity(.2)),
            ),
            child: Text(bias,
                style: GoogleFonts.dmSans(
        ...
        Text(text,
            style: GoogleFonts.dmSans(
                color: DS.textPrimary, fontSize: 13.5, height: 1.55)),
        const SizedBox(height: 16),
        ...
        Row(children: [
          ...
          Text('$sources/3 active',
              style: GoogleFonts.jetBrainsMono(
                  color: DS.textSecondary, fontSize: 10)),
        ]),
      ]),
    );
  }
...
class _SourceDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          color: active ? DS.green.withOpacity(.08) : DS.surfaceAlt,
          borderRadius: BorderRadius.circular(6),
          border:
              Border.all(color: active ? DS.green.withOpacity(.25) : DS.border),
        ),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Container(
            width: 5,
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              color: active ? DS.green : DS.textTertiary,
            ),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: sc.withOpacity(.2)),
            ),
            child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 16),
        Container(height: 1, color: DS.divider),
        const SizedBox(height: 14),
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(
                child: Text(summary.oneLiner,
                    style: GoogleFonts.dmSans(
                        color: DS.textSecondary, fontSize: 13, height: 1.55))),
          ]),
        ],
      ]),
    );
...
class _StatCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(
        ...
        children: [
          Text(label,
              style: GoogleFonts.dmSans(
                  color: DS.textTertiary, fontSize: 9, letterSpacing: .3)),
            ...
            Expanded(
                child: Column(
                  ...
                  Text('Yahoo Finance · ^NSEI',
                      style: GoogleFonts.dmSans(
                          color: DS.textTertiary, fontSize: 11)),
                ])),
            if (s5.isNotEmpty) ...[
              _MiniStat(
                  label: 'H',
            ...
            ? const SizedBox(
                ...
                child: Center(
                    child: Text('No intraday data',
                        style:
                            TextStyle(color: DS.textTertiary, fontSize: 12))))
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(
                  child: Text(summary.nifty5minInterpretation,
                      style: GoogleFonts.dmSans(
                          color: DS.textSecondary, fontSize: 12, height: 1.5))),
            ]),
          ),
        ] else
          const SizedBox(height: 14),
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: SfCartesianChart(
          ...
          primaryXAxis: DateTimeAxis(
            ...
            labelStyle: const TextStyle(color: DS.textTertiary, fontSize: 9),
          ),
          primaryYAxis: NumericAxis(
            numberFormat: NumberFormat.compact(),
            majorGridLines: const MajorGridLines(
                color: DS.divider, width: 1, dashArray: [3, 3]),
            axisLine: const AxisLine(width: 0),
            majorTickLines: const MajorTickLines(size: 0),
            labelStyle: const TextStyle(color: DS.textTertiary, fontSize: 9),
          ...
          series: [
            AreaSeries<CandlePoint, DateTime>(
              dataSource: candles,
              // Plot using IST-shifted times so the x-axis labels are correct
              ...
              gradient: LinearGradient(
                colors: [lineColor.withOpacity(.12), lineColor.withOpacity(0)],
                      ...
                      children: [
                        Text(DateFormat('HH:mm').format(istTime),
                            style: const TextStyle(
                                color: DS.textSecondary, fontSize: 9.5)),
...
class _VixCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ltp = (vix['ltp'] ?? 0.0) as double;
    final trend = vix['trend'] as String? ?? 'Sideways';
    final level = vix['level'] as String? ?? 'Moderate';
    final fear = vix['fear_flag'] as bool? ?? false;
    final interp = vix['interpretation'] as String? ?? '';
    final chgPct = (vix['day_change_pct'] ?? 0.0) as double;
    final isUp = chgPct >= 0;
    ...
    return Container(
      decoration: BoxDecoration(
        color: DS.surface,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: fear ? DS.red.withOpacity(.25) : DS.border),
        boxShadow: [
          BoxShadow(
              color: Colors.black.withOpacity(.04),
              blurRadius: 10,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (latest.isNotEmpty) ...[
          ...
          ...latest.map((s) {
            ...
            final fresh = s['_freshness'] as Map<String, dynamic>?;
            return Padding(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 8),
                if (fresh != null)
                  _FreshnessLabel(label: fresh['label'] as String? ?? ''),
                const SizedBox(width: 6),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        color: DS.surface,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: dc.withOpacity(.2)),
        boxShadow: [
          BoxShadow(
              color: Colors.black.withOpacity(.04),
              blurRadius: 10,
      ...
      child: Column(children: [
        // Header — only rendered for Long/Short, not Wait
        if (!isWait)
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            decoration: BoxDecoration(
              color: dc.withOpacity(.04),
              borderRadius:
        ...
        children: [
          Row(children: [
            Container(
              ...
              height: 36,
              decoration: BoxDecoration(
                color: DS.amber.withOpacity(.08),
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: DS.amber.withOpacity(.2)),
              ),
...
class _LiveIndicesCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (indices.isEmpty) return const _EmptyCard(message: 'No live index data');
    return Container(
      decoration: BoxDecoration(
        color: DS.surface,
        borderRadius: BorderRadius.circular(16),
      ...
      child: Column(
        ...
        return Column(children: [
          Padding(
            ...
            child: Row(children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: dispColor.withOpacity(.08),
          ...
          if (i < indices.length - 1)
            const Divider(
                height: 1, indent: 16, endIndent: 16, color: DS.divider),
        ]);
      })),
    );
  }
...
class _RiskCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _ListCard(
        ...
        iconColor: DS.red,
        borderColor: DS.red.withOpacity(.15),
      );
}
...
class _ListCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        decoration: BoxDecoration(
          color: DS.surface,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: borderColor ?? DS.border),
          boxShadow: [
            BoxShadow(
                color: Colors.black.withOpacity(.04),
                blurRadius: 10,
          ...
          Expanded(
              child: Column(
                ...
                Row(children: [
                  ...
                  const SizedBox(width: 6),
                  if (summary.ttlSeconds != null)
                    _Chip(
                        label: 'TTL ${summary.ttlSeconds}s',
                        color: DS.textTertiary),
                ]),
              ])),
          GestureDetector(
...
class _AnalysisCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
              color: actionable ? accent.withOpacity(.3) : DS.border),
          boxShadow: [
            if (actionable)
              BoxShadow(
...
class _ProbBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
        decoration: BoxDecoration(
          color: strong ? DS.green.withOpacity(.08) : DS.surfaceAlt,
          borderRadius: BorderRadius.circular(6),
          border:
              Border.all(color: strong ? DS.green.withOpacity(.2) : DS.border),
        ),
        child: Text('${prob.toStringAsFixed(1)}%${strong ? " ✓" : ""}',
            style: GoogleFonts.jetBrainsMono(
                color: strong ? DS.green : DS.textSecondary,
                fontSize: 11,
                fontWeight: FontWeight.w700)),
      );
}
...
class _AdviceRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Text(text,
            style: GoogleFonts.dmSans(
                color: DS.textPrimary, fontSize: 12.5, height: 1.4)),
      );
}
      ...
      child: Row(children: [
        ...
        Expanded(
            child: Text(signal['description'] ?? '',
                style: GoogleFonts.dmSans(
                    color: DS.textPrimary, fontSize: 12, height: 1.4))),
        const SizedBox(width: 6),
...
class _MiniStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label,
            style: const TextStyle(color: DS.textTertiary, fontSize: 9)),
        Text(value,
            style: GoogleFonts.jetBrainsMono(
                color: color ?? DS.textPrimary,
                fontSize: 11,
...
class _PriceBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3),
      decoration: BoxDecoration(
          color: cc.withOpacity(.08), borderRadius: BorderRadius.circular(6)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(
            ...
            style:
                GoogleFonts.dmSans(color: DS.amber, fontSize: 11, height: 1.4),
          )),
        ]),
      );
}
...
class _EmptyCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 8),
          Text(message,
              style: GoogleFonts.dmSans(color: DS.textSecondary, fontSize: 13)),
        ]),
      );
}
...
// ─── Entry button ─────────────────────────────────────────────────────────────
class NiftyAiSummaryButton extends StatelessWidget {
  const NiftyAiSummaryButton({super.key});
  @override
  Widget build(BuildContext context) => GestureDetector(
        onTap: () => Navigator.push(
            context,
            PageRouteBuilder(
              pageBuilder: (_, a, __) => const NiftyAiSummaryPage(),
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                  color: DS.blue.withOpacity(.2),
                  blurRadius: 16,
                  offset: const Offset(0, 6))
            ],
          ),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            const Icon(Icons.show_chart_rounded, color: Colors.white, size: 18),
            const SizedBox(width: 10),
            Text('Nifty AI Summary',
                style: GoogleFonts.dmSans(
                    color: Colors.white,
