...
import 'package:optionxi/Main_Pages/BrokersPage/Upstox/homepage_upstox.dart';
...
// ─────────────────────────────────────────────────────────────
class _UpstoxTheme {
  static const upstoxPurple = Color(0xFF5D3FD3);
...
//  Main Widget
// ─────────────────────────────────────────────────────────────
class UpstoxConnectPage extends StatefulWidget {
  const UpstoxConnectPage({Key? key}) : super(key: key);

  @override
  State<UpstoxConnectPage> createState() => _UpstoxConnectPageState();
}

class _UpstoxConnectPageState extends State<UpstoxConnectPage>
  ...
  Future<void> _initializePage() async {
    ...
    if (user != null) {
      _suid = user.uid;
      _dbRef = FirebaseDatabase.instance.ref().child('brokers/upstox/$_suid');
  ...
  Future<void> _saveCredentials() async {
    ...
    try {
      ...
      _showToast('Credentials saved — now authenticate with Upstox');
    } catch (e) {
  ...
  Future<void> _launchUpstoxAuthentication() async {
    ...
    final url = Uri.parse(
        'https://api.upstox.com/v2/login/authorization/dialog?response_type=code&client_id=${_apiKeyController.text.trim()}&redirect_uri=https://fastapi.optionxi.com/broker/upstox&state=$suid');
  ...
  Future<bool?> _showConfirmDialog() => showDialog<bool>(
        ...
        builder: (ctx) {
          ...
          return AlertDialog(
            backgroundColor: _UpstoxTheme.card(dark),
            shape:
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: _UpstoxTheme.errorRed.withOpacity(0.12),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: const Icon(Icons.link_off_rounded,
                      color: _UpstoxTheme.errorRed, size: 20),
                ),
            ...
            content: Text(
              'This will delete your stored credentials and disconnect your Upstox account permanently.',
              style: TextStyle(color: _UpstoxTheme.textSecondary(dark)),
            ),
            actions: [
              TextButton(
              ...
              FilledButton(
                ...
                style: FilledButton.styleFrom(
                    backgroundColor: _UpstoxTheme.errorRed,
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                height: 4,
                decoration: BoxDecoration(
                  color: _UpstoxTheme.cardBorder(dark),
              ...
              Stack(
                ...
                children: [
                  Container(
                    ...
                    height: 80,
                    decoration: BoxDecoration(
                      color: _UpstoxTheme.successGreen.withOpacity(0.08),
                  ...
                  Container(
                    ...
                    height: 60,
                    decoration: BoxDecoration(
                      color: _UpstoxTheme.successGreen.withOpacity(0.15),
                      shape: BoxShape.circle,
                    ),
                  ),
                  const Icon(Icons.check_rounded,
                      color: _UpstoxTheme.successGreen, size: 32),
                ],
              ),
              ...
              Text(
                ...
                style: TextStyle(
                  ...
                  color: _UpstoxTheme.textPrimary(dark),
                ),
              ),
              const SizedBox(height: 8),
              Text(
                'Your Upstox account is linked and ready.\nStart trading now.',
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: 15,
                  color: _UpstoxTheme.textSecondary(dark),
                  height: 1.5,
                ),
              ),
              const SizedBox(height: 28),
                ...
                child: FilledButton.icon(
                  onPressed: () {
                    ...
                    Navigator.push(context,
                        MaterialPageRoute(builder: (_) => HomepageUpstox()));
                  },
                  icon: const Icon(FontAwesomeIcons.chartArea, size: 16),
                  label: const Text('Start Trading',
                      style:
                          TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                  style: FilledButton.styleFrom(
                    backgroundColor: _UpstoxTheme.successGreen,
  ...
  void _showToast(String message, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      backgroundColor:
          isError ? _UpstoxTheme.errorRed : const Color(0xFF1E293B),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: _UpstoxTheme.surface(dark),
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  ApiKeyStepsSection(brokerName: "upstox"),
                  const SizedBox(height: 16),
          ...
          children: [
            Container(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child:
                    Image.asset('assets/brokers/upstox.png', fit: BoxFit.cover),
              ),
            ),
            const SizedBox(width: 10),
            Text(
        ...
        actions: [
          IconButton(
            icon: Icon(Icons.refresh_rounded,
                color: _UpstoxTheme.textSecondary(dark), size: 20),
  ...
  Widget _buildLiveDot(bool dark) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isActive
            ? _UpstoxTheme.successGreen
            : isPending
                ? _UpstoxTheme.warningAmber
                : _UpstoxTheme.textSecondary(dark).withOpacity(0.4),
        shape: BoxShape.circle,
      ),
    );
  }
  ...
  Map<String, dynamic> _statusConfig(bool dark) {
    switch (_status) {
      case 'authenticated':
        return {
          'bg': _UpstoxTheme.successGreen.withOpacity(dark ? 0.1 : 0.06),
          'accent': _UpstoxTheme.successGreen,
          'icon': Icons.verified_rounded,
          'label': 'CONNECTED',
        };
      case 'credentials_saved':
        return {
          'bg': _UpstoxTheme.upstoxPurple.withOpacity(dark ? 0.1 : 0.06),
          'accent': _UpstoxTheme.upstoxPurple,
          'icon': Icons.task_alt_rounded,
          'label': 'CREDENTIALS SAVED',
        };
      case 'pending':
        return {
          'bg': _UpstoxTheme.warningAmber.withOpacity(dark ? 0.1 : 0.06),
          'accent': _UpstoxTheme.warningAmber,
      ...
      case 'error':
        return {
          'bg': _UpstoxTheme.errorRed.withOpacity(dark ? 0.1 : 0.06),
          'accent': _UpstoxTheme.errorRed,
          'icon': Icons.error_outline_rounded,
          'label': 'FAILED',
        };
      default:
        return {
          'bg': _UpstoxTheme.subtle(dark),
          'accent': _UpstoxTheme.textSecondary(dark),
  ...
  Widget _buildStepFlow(bool dark) {
    final steps = [
      ...
      _StepInfo(
        number: '2',
        title: 'Authenticate',
        subtitle: 'Login via Upstox OAuth',
          ...
          return Expanded(
            child: Container(
              ...
              color: leftStep.isDone
                  ? _UpstoxTheme.upstoxPurple.withOpacity(0.5)
                  : _UpstoxTheme.cardBorder(dark),
            ),
          );
        }
  ...
  Widget _buildStepDot(_StepInfo step, bool dark) {
    ...
    if (step.isDone) {
      accentColor = _UpstoxTheme.upstoxPurple;
      ...
      accentColor = _UpstoxTheme.cardBorder(dark);
      icon = Text(step.number,
          style: TextStyle(
              color: _UpstoxTheme.textSecondary(dark),
              fontSize: 12,
        ...
        Text(
          ...
          style: TextStyle(
            ...
            color: step.isActive || step.isDone
                ? _UpstoxTheme.textPrimary(dark)
                : _UpstoxTheme.textSecondary(dark),
          ),
        ),
      ],
    );
  ...
  Widget _buildStartTradingCard(bool dark) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: _UpstoxTheme.successGreen.withOpacity(0.3),
            blurRadius: 20,
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () => Navigator.push(
                context, MaterialPageRoute(builder: (_) => HomepageUpstox())),
            icon: const Icon(FontAwesomeIcons.chartArea, size: 14),
            label: const Text('Trade',
                style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
            style: FilledButton.styleFrom(
              backgroundColor: Colors.white,
              foregroundColor: _UpstoxTheme.successGreen,
  ...
  Widget _buildCredentialsCard(bool dark) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _UpstoxTheme.card(dark),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: _UpstoxTheme.cardBorder(dark)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: _UpstoxTheme.upstoxPurple.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(6),
                ),
                child: const Text('Upstox Developer',
                    style: TextStyle(
                        fontSize: 10,
                        color: _UpstoxTheme.upstoxPurple,
                        fontWeight: FontWeight.w600)),
              ),
            ],
          ),
          const SizedBox(height: 4),
          Text(
            'Find these in your Upstox Developer dashboard',
            style: TextStyle(
                fontSize: 12, color: _UpstoxTheme.textSecondary(dark)),
          ),
          const SizedBox(height: 20),
          _buildInputField(
        ...
        AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: isFocused
                  ? _UpstoxTheme.upstoxPurple
                  : _UpstoxTheme.inputBorder(dark),
              width: isFocused ? 1.5 : 1,
            ),
            color: _UpstoxTheme.inputFill(dark),
          ),
          child: TextFormField(
            ...
            decoration: InputDecoration(
              hintText: hint,
              hintStyle: TextStyle(
                  color: _UpstoxTheme.textSecondary(dark).withOpacity(0.5),
                  fontSize: 14),
              prefixIcon: Padding(
      ...
      return Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text('Saving…',
                style: TextStyle(
                    color: _UpstoxTheme.textSecondary(dark), fontSize: 14)),
          ],
        ),
      );
    }
    ...
    switch (_status) {
      ...
      case 'connection_failed':
        return Column(
          children: [
            _primaryButton(
              label: 'Authenticate with Upstox',
              icon: Icons.open_in_browser_rounded,
              color: _UpstoxTheme.upstoxPurple,
              onTap: _launchUpstoxAuthentication,
            ),
  ...
  Widget _buildRedirectCard(bool dark) {
    const redirectUrl = 'https://fastapi.optionxi.com/broker/upstox';
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: _UpstoxTheme.card(dark),
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Container(
              ...
              child: const Icon(Icons.link_rounded,
                  color: _UpstoxTheme.upstoxPurple, size: 18),
            ),
            const SizedBox(width: 12),
            Text('Redirect URL',
                style: TextStyle(
          ...
          const SizedBox(height: 10),
          Text(
            'Add this URL as the redirect URI in your Upstox Developer app settings.',
            style: TextStyle(
                fontSize: 13,
                color: _UpstoxTheme.textSecondary(dark),
                height: 1.5),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: _UpstoxTheme.subtle(dark),
                borderRadius: BorderRadius.circular(10),
                border: Border.all(
                  color: _UpstoxTheme.upstoxPurple.withOpacity(0.2),
                ),
              ),
              child: Row(
                children: [
                  ...
                  Icon(Icons.copy_rounded,
                      size: 16, color: _UpstoxTheme.textSecondary(dark)),
                ],
              ),
            ),
          ),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: () async {
                final url =
                    Uri.parse('https://account.upstox.com/developer/apps');
                if (await canLaunchUrl(url)) {
                  await launchUrl(url, mode: LaunchMode.externalApplication);
                }
              },
              icon: const Icon(Icons.open_in_new_rounded, size: 15),
              label: const Text('Open Upstox Developer Portal',
                  style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
              style: OutlinedButton.styleFrom(
                foregroundColor: _UpstoxTheme.upstoxPurple,
                side: BorderSide(
                    color: _UpstoxTheme.upstoxPurple.withOpacity(0.4)),
      ...
      child: Row(
        ...
        children: [
          Container(
            padding: const EdgeInsets.all(8),
            decoration: BoxDecoration(
              color: _UpstoxTheme.successGreen.withOpacity(0.1),
              borderRadius: BorderRadius.circular(10),
            ),
            child: const Icon(Icons.shield_rounded,
                color: _UpstoxTheme.successGreen, size: 18),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'Watch our step-by-step guide to connect Upstox',
            style: TextStyle(
                fontSize: 13, color: _UpstoxTheme.textSecondary(dark)),
          ),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(
                child: FilledButton.icon(
                  onPressed: () async {
                    final url = Uri.parse(
                        'https://www.youtube.com/watch?v=XZH3j2P07mo');
                    if (await canLaunchUrl(url)) {
                      await launchUrl(url,
                ...
                child: FilledButton.icon(
                  ...
                  label: const Text('Support',
                      style:
                          TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                  style: FilledButton.styleFrom(
                    backgroundColor: _UpstoxTheme.upstoxPurple,
                    foregroundColor: Colors.white,
                    padding: const EdgeInsets.symmetric(vertical: 12),
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(10)),
  ...
  Widget _buildSectionDivider(String label, bool dark) {
    return Row(children: [
      Expanded(child: Divider(color: _UpstoxTheme.cardBorder(dark))),
      Padding(
            ...
            child: Column(
              ...
              children: [
                Container(
                  padding: const EdgeInsets.all(20),
                  decoration: BoxDecoration(
                    color: _UpstoxTheme.errorRed.withOpacity(0.1),
                    shape: BoxShape.circle,
                  ),
                  child: const Icon(Icons.account_circle_outlined,
                      color: _UpstoxTheme.errorRed, size: 48),
                ),
                const SizedBox(height: 24),
                ...
                const SizedBox(height: 8),
                Text(
                  'Please sign in to connect your Upstox account.',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                      fontSize: 14,
                      color: _UpstoxTheme.textSecondary(dark),
