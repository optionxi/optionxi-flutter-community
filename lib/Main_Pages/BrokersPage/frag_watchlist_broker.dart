...
import 'package:optionxi/Components/cust_watchlist_item_broker.dart';
...
class WatchlistFragmentBroker extends StatefulWidget {
  ...
  const WatchlistFragmentBroker({
    Key? key,
    required this.whichbroker,
  }) : super(key: key);

  @override
  State<WatchlistFragmentBroker> createState() =>
      _WatchlistFragmentBrokerState();
}

class _WatchlistFragmentBrokerState extends State<WatchlistFragmentBroker> {
              ...
              child: NestedScrollView(
                ...
                body: Obx(() {
                  if (watchlistController.isLoading.value) {
                    return const Center(child: WatchlistShimmerLoader());
                  }
                  ...
                  return RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return WatchlistItemBroker(
                          stock: stock,
                          whichbroker: widget.whichbroker,
                        );
                      },
                    ),
                  );
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          ElevatedButton.icon(
            onPressed: () => watchlistController.loadWatchlist(),
...
class _WatchlistShimmerLoaderState extends State<WatchlistShimmerLoader>
  ...
  Widget _buildShimmerContainer({
    ...
    final double sweep = _animation.value; // ranges from -1.0 to 2.0
    return Container(
      height: height,
      width: width,
      margin: margin,
...
// -------------------------------------------------------------
// Placeholder Dummy Pages for the Respective Buy/Sell Navigation.
// (You can replace these implementations with your actual files
// or remove them if you already have these globally accessible)
// -------------------------------------------------------------

class BrokerBuyPage extends StatelessWidget {
  final dynamic stock;
  final String whichbroker;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Text(
          'Executing Buy Order via $whichbroker',
...
class BrokerSellPage extends StatelessWidget {
  final dynamic stock;
  final String whichbroker;

  const BrokerSellPage({
    Key? key,
    required this.stock,
    required this.whichbroker,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Text(
          'Executing Sell Order via $whichbroker',
          style: GoogleFonts.poppins(fontSize: 18),
        ),
      ),
    );
