...
// ─────────────────────────────────────────────────────────────
//  UpstoxOrderEditPage
//  Modify:  PUT  /upstox/orders/{order_id}
//           body: { order_id, quantity, price, trigger_price, order_type, validity }
//  Cancel:  DELETE /upstox/orders/{order_id}
//
//  Upstox product codes : I (Intraday/MIS)  D (Delivery/CNC)  CO  OCO
//  Order types (string) : MARKET | LIMIT | SL | SL-M
//  Validity             : DAY | IOC
//  instrument_token     : "NSE_EQ|INE155A01022"  (stored as instrument_key)
// ─────────────────────────────────────────────────────────────

// ── Upstox-specific order data model ─────────────────────────
class UpstoxOrderModel {
  final String orderId; // "order_id" from Upstox response
  final String instrumentToken; // "NSE_EQ|INE155A01022"
  final String tradingSymbol; // display name e.g. "RELIANCE"
  final String transactionType; // "BUY" | "SELL"
  final String orderType; // "MARKET" | "LIMIT" | "SL" | "SL-M"
  final int quantity;
  final double price; // limit price
  final double triggerPrice; // SL trigger
  final String validity; // DAY | IOC
  final String product; // I | D | CO | OCO
  final String status; // open | complete | cancelled | rejected
  final double averagePrice;
  final String exchange; // NSE | BSE

  const UpstoxOrderModel({
  ...
  /// Human-readable label for Upstox product code
  static String productLabel(String code) {
...
class UpstoxOrderEditPage extends StatefulWidget {
  final UpstoxOrderModel order;

  const UpstoxOrderEditPage({Key? key, required this.order}) : super(key: key);

  @override
  State<UpstoxOrderEditPage> createState() => _UpstoxOrderEditPageState();
}

class _UpstoxOrderEditPageState extends State<UpstoxOrderEditPage>
  ...
  // Upstox: LIMIT and SL need price; SL and SL-M need trigger_price
  ...
  Future<void> _modifyOrder() async {
    ...
    try {
      ...
      // Upstox PUT /upstox/orders/{order_id}
      // Server adds order_id to body and calls PUT /v2/order/modify
      ...
      final response = await dio.put(
        '$baseUrl/upstox/orders/${widget.order.orderId}',
  ...
  Future<void> _cancelOrder() async {
    ...
    try {
      ...
      // Upstox DELETE /upstox/orders/{order_id}
      // Server calls DELETE /v2/order/cancel?order_id=...
  ...
  Future<bool> _showCancelConfirm() async {
    ...
    return await showDialog<bool>(
          ...
          builder: (_) => _UpCancelConfirmDialog(
            isDark: isDark,
                  ...
                  child: Form(
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        // ── Instrument token info ──────────────
                        _UpTokenBadge(
                          isDark: isDark,
                          instrumentToken: widget.order.instrumentToken,
                          exchange: widget.order.exchange,
                        ),
...
class _UpCancelConfirmDialog extends StatelessWidget {
  final bool isDark;
  final String symbol;
  final String orderId;

  const _UpCancelConfirmDialog({
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'This will cancel your $symbol order on Upstox. This action cannot be undone.',
...
// ═══════════════════════════════════════════════════════════════
//  UPSTOX ORDER INFO CARD  (read-only)
// ═══════════════════════════════════════════════════════════════

class _UpOrderInfoCard extends StatelessWidget {
  final UpstoxOrderModel order;
  final bool isDark;
  final Color activeColor;

  const _UpOrderInfoCard({
    required this.order,
    required this.isDark,
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                  child: _kv('Product',
                      UpstoxOrderModel.productLabel(order.product), isDark)),
...
// ═══════════════════════════════════════════════════════════════
//  INSTRUMENT TOKEN BADGE  (Upstox-specific)
// ═══════════════════════════════════════════════════════════════

class _UpTokenBadge extends StatelessWidget {
  final bool isDark;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.token_outlined, size: 14, color: accent),
          const SizedBox(width: 8),
          Text('Instrument: ',
              style: TextStyle(
                  fontSize: 12,
...
class _UpDiffCard extends StatelessWidget {
  final bool isDark;
  final UpstoxOrderModel original;
...
// ═══════════════════════════════════════════════════════════════
//  SHARED SMALL WIDGETS (Upstox-scoped with _Up prefix)
// ═══════════════════════════════════════════════════════════════

class _UpLabel extends StatelessWidget {
  final String label;
  final bool isDark;
  final IconData? icon;

  const _UpLabel({required this.label, required this.isDark, this.icon});
...
class _UpChips extends StatelessWidget {
...
class _UpEditQtyRow extends StatefulWidget {
  ...
  @override
  State<_UpEditQtyRow> createState() => _UpEditQtyRowState();
}

class _UpEditQtyRowState extends State<_UpEditQtyRow> {
...
class _UpPriceField extends StatelessWidget {
  final String label;
  final TextEditingController controller;
  final bool isDark;
