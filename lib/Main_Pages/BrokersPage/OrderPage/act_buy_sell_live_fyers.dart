...
// Fyers has `symbol` (clean), `fyers_symbol` (NSE:RELIANCE-EQ), `raw_symbol`
String _sym(Map<String, dynamic> s) =>
    (s['symbol'] ?? s['short_symbol'] ?? s['raw_symbol'] ?? '')
        .toString()
        .trim();
...
// ─────────────────────────────────────────────────────────────
//  Fyers product / order type definitions
// ─────────────────────────────────────────────────────────────

// Fyers product codes
const _fyersProducts = ['INTRADAY', 'CNC', 'MARGIN'];
const _fyersProductLabels = {'INTRADAY': 'MIS', 'CNC': 'CNC', 'MARGIN': 'NRML'};
const _fyersProductHints = {
  'INTRADAY': 'Intraday · auto-squares at 3:20 PM',
  'CNC': 'Delivery · held in demat',
  'MARGIN': 'F&O overnight carry-forward',
};
...
class FyersOrderPage extends StatefulWidget {
  final String? stockname;
  final String? segment;
  final bool tosell;

  const FyersOrderPage(
      {Key? key, this.stockname, this.segment, this.tosell = false})
      : super(key: key);

  @override
  State<FyersOrderPage> createState() => _FyersOrderPageState();
}

class _FyersOrderPageState extends State<FyersOrderPage>
  ...
  // Fyers API needs the fully-qualified symbol: "NSE:RELIANCE-EQ"
  String get _fyersSymbol =>
      (_stock?['fyers_symbol'] as String?) ??
      (_stock?['symbol'] as String?) ??
      '';

  // Exchange derived from fyers_symbol prefix or doc field
  String get _exchangeLabel {
    final fs = _fyersSymbol;
    if (fs.startsWith('NSE')) return 'NSE';
    if (fs.startsWith('BSE')) return 'BSE';
    if (fs.startsWith('MCX')) return 'MCX';
    return (_stock?['exchange'] as String?) ?? 'NSE';
  }

  // Fyers order type as string (server maps to int)
  ...
  void initState() {
    ...
    if (widget.stockname != null) {
      _stock = {
        ...
        'fyers_symbol': widget.segment == 'FNO'
            ? 'NSE:${widget.stockname}'
            : 'NSE:${widget.stockname}-EQ',
        'name': widget.stockname,
        'type': widget.segment == 'FNO' ? 'future' : 'stock',
        'exchange': widget.segment == 'FNO' ? 'NSE' : 'NSE',
      };
      if (widget.segment == 'FNO') _product = 'MARGIN';
  ...
  Future<void> _openSearch() async {
    final result = await Navigator.push<Map<String, dynamic>>(
      context,
      PageRouteBuilder(
        pageBuilder: (_, a1, a2) => _FyersSearchPage(
          meiliClient: meili.MeiliSearchClient(
              dotenv.env['MELIESEARCH_URL']!, dotenv.env['MELIE_API_KEY']!),
        ),
  ...
  Future<void> _fetchMargin() async {
    if (_stock == null || _fyersSymbol.isEmpty) return;
    setState(() {
      _loadingMargin = true;
      _margin = null;
    });
    try {
      ...
      // POST /fyers/margins/orders  — Fyers: data.marginUsed
      try {
        marginResp = await dio.post(
          '$baseUrl/fyers/margins/orders',
      ...
      // GET /fyers/user/funds  — Fyers: data.fund_limit[id==11].equityAmount
      try {
        fundsResp = await dio.get(
          '$baseUrl/fyers/user/funds',
          options: Options(headers: {'Authorization': 'Bearer $token'}),
        );
      } catch (_) {}

      // POST /fyers/charges/orders  — mirrors Zerodha shape
      try {
        chargesResp = await dio.post(
          '$baseUrl/fyers/charges/orders',
          options: Options(headers: {'Authorization': 'Bearer $token'}),
          data: {
            'symbol': _fyersSymbol,
            'transaction_type': _mode,
      ...
      // Fyers margin: data.marginUsed
      if (marginResp?.statusCode == 200) {
        final d = marginResp!.data?['data'];
        req = ((d?['marginUsed'] ?? 0) as num).toDouble();
      }

      // Fyers funds: data.fund_limit — find id==11 for available cash
      if (fundsResp?.statusCode == 200) {
        final fundList = fundsResp!.data?['data']?['fund_limit'];
        if (fundList is List) {
          final entry =
              fundList.firstWhere((e) => e['id'] == 11, orElse: () => null);
          if (entry != null)
            avail = ((entry['equityAmount'] ?? 0) as num).toDouble();
        }
      }

      // Charges: mirrors Zerodha shape (estimated by server)
      if (chargesResp?.statusCode == 200) {
        final ch = chargesResp!.data?['data'];
        brok = ((ch?['brokerage'] ?? 0) as num).toDouble();
        stt = ((ch?['taxes']?['stt'] ?? 0) as num).toDouble();
        etxn = ((ch?['taxes']?['exchange_txn_charge'] ?? 0) as num).toDouble();
        gst = ((ch?['taxes']?['gst'] ?? 0) as num).toDouble();
        sebi = ((ch?['taxes']?['sebi_charges'] ?? 0) as num).toDouble();
        stamp = ((ch?['taxes']?['stamp_duty'] ?? 0) as num).toDouble();
      }
      ...
      debugPrint('Fyers margin error: $e');
      if (mounted) setState(() => _loadingMargin = false);
    }
  }

  Future<void> _placeOrder() async {
    if (_stock == null || _fyersSymbol.isEmpty) {
    ...
    try {
      ...
      // Fyers order body — server translates to numeric type/side
      final body = <String, dynamic>{
        'symbol': _fyersSymbol, // "NSE:RELIANCE-EQ"
        'transaction_type': _mode, // "BUY" | "SELL"
        'order_type': _orderTypeApi, // "MARKET" | "LIMIT" | "SL" | "SL-M"
        'quantity': int.tryParse(_qtyCtrl.text) ?? 1,
        'product': _product, // "INTRADAY" | "CNC" | "MARGIN"
        'price': 0.0,
        'stop_price': 0.0,
        'validity': _validity,
        'disclosed_qty': 0,
      ...
      if (_needsTrigger) {
        ...
        if (tp != null)
          body['stop_price'] = tp; // Fyers uses stop_price for trigger
      }
  ...
  Widget _buildInstrumentTile(_T t) => GestureDetector(
        onTap: _openSearch,
        child: Container(
          color: t.sheet,
  ...
  Widget _filledInstrument(_T t) {
    ...
    final strike = (_stock!['strike'] as num?)?.toDouble();
    ...
    return Row(children: [
      Expanded(
        ...
        // Show fyers_symbol as secondary label for stocks
        if (!(isOption || isFuture) && _fyersSymbol.isNotEmpty) ...[
          const SizedBox(height: 3),
          Text(_fyersSymbol,
              style:
                  TextStyle(fontSize: 11, color: t.t3, fontFamily: 'monospace'),
              maxLines: 1,
              overflow: TextOverflow.ellipsis),
  ...
  Widget _buildProductSection(_T t) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 8),
      Row(
          children: _fyersProducts.asMap().entries.map((e) {
        final p = e.value;
        final isLast = e.key == _fyersProducts.length - 1;
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
                ...
                border: Border.all(
                    ...
                    width: active ? 1.5 : 1)),
            child: Text(_fyersProductLabels[p] ?? p,
      ...
      Text(_fyersProductHints[_product] ?? '',
          style: TextStyle(fontSize: 11, color: t.t3)),
    ]);
  }
  ...
  Widget _buildExchangeRow(_T t) {
    ...
    return Row(children: [
      ...
      Text('From symbol', style: TextStyle(fontSize: 10, color: t.t3)),
    ]);
  }
  ...
  Widget _buildAdvancedToggle(_T t) => GestureDetector(
  ...
  Widget _buildAdvancedContent(_T t) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _sumRow(t, 'Fyers symbol', _fyersSymbol.isEmpty ? '—' : _fyersSymbol),
        _sumRow(t, 'Mode', _mode),
        _sumRow(t, 'Exchange', _exchangeLabel),
        _sumRow(t, 'Product',
            '${_fyersProductLabels[_product] ?? _product} ($_product)'),
        ...
        if (trigger != null && trigger > 0)
          _sumRow(t, 'Stop price', '₹${trigger.toStringAsFixed(2)}'),
      ]),
    );
  }
  ...
  Widget _buildBottomBar(_T t) => SafeArea(
        top: false,
        child: Container(
          decoration: BoxDecoration(
            ...
            SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _placing
                      ...
                      : Text('${_mode == 'BUY' ? 'Buy' : 'Sell'} via Fyers',
                ...
                AnimatedSize(
                  ...
                  child: expanded
                      ? Column(children: [
                          ...
                          _cRow(t, 'Exchange transaction charge',
                              _f(margin!.exchTxn)),
                          _cRow(t, 'GST (18%)', _f(margin!.gst)),
                          _cRow(t, 'SEBI charges', _f(margin!.sebi)),
...
// ═══════════════════════════════════════════════════════════════
//  FYERS SEARCH PAGE  —  index: stocks_fyers
// ═══════════════════════════════════════════════════════════════

class _FyersSearchPage extends StatefulWidget {
  final meili.MeiliSearchClient meiliClient;
  const _FyersSearchPage({required this.meiliClient});
  @override
  State<_FyersSearchPage> createState() => _FyersSearchPageState();
}

class _FyersSearchPageState extends State<_FyersSearchPage> {
  ...
  Future<void> _search(String q) async {
    ...
    try {
      ...
      // Fyers index: stocks_fyers
      final res = await widget.meiliClient.index('stocks_fyers').search(
          q.trim(),
          meili.SearchQuery(
            filter: filters.isNotEmpty ? filters.join(' AND ') : null,
            attributesToRetrieve: [
              'id', 'symbol', 'short_symbol', 'raw_symbol', 'name', 'exchange',
              'type',
              'option_type', 'expiry', 'expiry_ts', 'strike', 'lot_size',
              'fyers_symbol', // ← fully qualified: "NSE:RELIANCE-EQ"
              'underlying_script_code', 'segment',
            ],
...
// ─────────────────────────────────────────────────────────────
//  Fyers search result tile  —  shows fyers_symbol as tag
// ─────────────────────────────────────────────────────────────
...
class _SearchResultTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final strike = (doc['strike'] as num?)?.toDouble();
    ...
    final isOption = type == 'option';
    final isFuture = type == 'future';
    final fyersSym = (doc['fyers_symbol'] as String?) ?? '';
        ...
        child: Row(children: [
          Expanded(
              child: Column(
                ...
                // Show fully-qualified Fyers symbol as monospace tag
                if (fyersSym.isNotEmpty) ...[
                  const SizedBox(height: 2),
                  Text(fyersSym,
                      style: TextStyle(
                          fontSize: 10,
  ...
  Future<void> _onChanged(String q) async {
    ...
    try {
      final res = await widget.meiliClient.index('stocks_fyers').search(
          ...
          meili.SearchQuery(
              filter: 'type = "option"',
              attributesToRetrieve: ['symbol', 'short_symbol'],
              limit: 50));
      final hits = res.hits.cast<Map<String, dynamic>>();
      final seen = <String>{};
      ...
      for (final h in hits) {
        final ss = (h['short_symbol'] as String?)?.toUpperCase();
        if (ss != null && seen.add(ss)) {
          underlyings.add(ss);
          continue;
        }
  ...
  Future<void> _fetch(String underlying) async {
    ...
    try {
      ...
      for (final h in filtered.isNotEmpty ? filtered : hits) {
        ...
        final strike = (h['strike'] as num?)?.toDouble();
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                                color:
                                    active ? _T.ce.withOpacity(0.12) : t.chip,
                                borderRadius: BorderRadius.circular(8),
                                border: Border.all(
                                    color: active
