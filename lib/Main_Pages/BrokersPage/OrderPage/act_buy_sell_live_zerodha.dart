...
String _sym(Map<String, dynamic> s) =>
    (s['symbol'] ?? s['tradingsymbol'] ?? '').toString().trim();
...
const _orderTypeDesc = {
  'Market':
      'Executes immediately at the best available price. No price control.',
  'Limit': 'Executes only at your specified price or better.',
  'SL': 'Stop-loss limit — triggers at stop price, executes at limit price.',
  'SL-M':
      'Stop-loss market — triggers at stop price, exits at market immediately.',
};
...
class ZerodhaOrderPage extends StatefulWidget {
  final String? stockname;
  final String? segment;
  final bool tosell;

  const ZerodhaOrderPage(
      {Key? key, this.stockname, this.segment, this.tosell = false})
      : super(key: key);

  @override
  State<ZerodhaOrderPage> createState() => _ZerodhaOrderPageState();
}

class _ZerodhaOrderPageState extends State<ZerodhaOrderPage>
  ...
  static const _products = ['MIS', 'CNC', 'NRML'];
  static const _validities = ['DAY', 'IOC'];
  static const _productHints = {
    'MIS': 'Intraday · auto-squares at 3:20 PM',
    'CNC': 'Delivery · held in demat',
    'NRML': 'F&O overnight carry-forward',
  };
  ...
  String get _tradingSymbol =>
      (_exchange == 'NFO' || _exchange == 'MCX' || _exchange == 'CDS')
          ? (_stock?['tradingsymbol'] ?? '').toString()
          : (_stock != null ? _sym(_stock!) : '');
  ...
  void initState() {
    ...
    if (widget.stockname != null) {
      _stock = {
        ...
        'tradingsymbol': widget.stockname,
        'name': widget.stockname,
        'type': widget.segment == 'FNO' ? 'future' : 'stock',
        'exchange': widget.segment == 'FNO' ? 'NFO' : 'NSE',
      };
  ...
  Future<void> _openSearch() async {
    final result = await Navigator.push<Map<String, dynamic>>(
      context,
      PageRouteBuilder(
        pageBuilder: (_, a1, a2) => _SearchPage(
  ...
  void _applyStock(Map<String, dynamic> s) {
    setState(() {
      _stock = s;
      final exch = (s['exchange'] as String?) ?? 'NSE';
      _exchange = exch;
  ...
  Future<void> _fetchMargin() async {
    ...
    try {
      ...
      final sym = _tradingSymbol;
      final otApi = _orderType == 'Market'
          ? 'MARKET'
          : _orderType == 'Limit'
              ? 'LIMIT'
              : _orderType;
      ...
      try {
        fundsResp = await dio.get(
          '$baseUrl/user/margins',
          options: Options(headers: {'Authorization': 'Bearer $token'}),
        );
      } catch (_) {}

      try {
        chargesResp = await dio.get(
          '$baseUrl/charges/orders',
          options: Options(headers: {'Authorization': 'Bearer $token'}),
          queryParameters: {
            'exchange': _exchange,
            'tradingsymbol': sym,
      ...
      if (marginResp?.statusCode == 200) {
        ...
        if (d is List && d.isNotEmpty) {
          req = ((d[0]['total']?['with_opt_chain'] ??
                  d[0]['total']?['wo_opt_chain'] ??
                  0) as num)
              .toDouble();
        }
      }
      if (fundsResp?.statusCode == 200) {
        final seg =
            (_exchange == 'NFO' || _exchange == 'MCX' || _exchange == 'CDS')
                ? 'commodity'
                : 'equity';
        final funds = fundsResp!.data?['data']?[seg];
        avail = ((funds?['available']?['live_balance'] ?? 0) as num).toDouble();
      }
  ...
  // ── Header — BUY/SELL toggle is now taller (44px, padded) ───
        ...
        Expanded(
          ...
          Text('Place Order',
              style: TextStyle(
                  ...
                  color: t.t1,
                  letterSpacing: -0.3)),
          Text('Zerodha · Live', style: TextStyle(fontSize: 11, color: t.t3)),
        ])),
        const SizedBox(width: 12),
        _BsToggle(
          t: t,
  ...
  // CHANGED: expiry/strike row only shown for options/futures, not stocks
  Widget _filledInstrument(_T t) {
    ...
    return Row(children: [
      Expanded(
        ...
        // Subtitle: expiry+strike for F&O; company name for stocks (omitted if empty)
        if (isOption || isFuture) ...[
  ...
  Widget _buildExchangeRow(_T t) {
    ...
    switch (_exchange) {
      case 'NSE':
        ...
        break;
      case 'BSE':
        ec = const Color(0xFF34D399);
        break;
      case 'NFO':
        ec = _T.amber;
        break;
      case 'MCX':
        ec = const Color(0xFFF97316);
        break;
      case 'CDS':
        ec = const Color(0xFFA78BFA);
        break;
      default:
        ec = _T.blue;
    }
    return Row(children: [
      ...
      Text('Auto-set', style: TextStyle(fontSize: 10, color: t.t3)),
    ]);
  }

  Widget _buildMarginCard(_T t) {
    return _MarginCard(
      t: t,
      ac: _ac,
      ...
      onToggleExpand: () => setState(() => _marginExpanded = !_marginExpanded),
    );
  }
  ...
  Widget _buildAdvancedContent(_T t) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _sumRow(t, 'Symbol', _tradingSymbol.isEmpty ? '—' : _tradingSymbol),
        _sumRow(t, 'Mode', _mode),
        _sumRow(t, 'Exchange', _exchange),
        _sumRow(t, 'Product', _product),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: _placing
                  ...
                  : Text(
                      '${_mode == 'BUY' ? 'Buy' : 'Sell'} via Zerodha',
...
// ═══════════════════════════════════════════════════════════════
//  BS TOGGLE  — taller pill (44px) with generous horizontal padding
// ═══════════════════════════════════════════════════════════════
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              padding: const EdgeInsets.symmetric(
                  horizontal: 22, vertical: 7), // CHANGED: wider + taller
              decoration: BoxDecoration(
                color: active ? color.withOpacity(0.14) : Colors.transparent,
                borderRadius: BorderRadius.circular(9),
                border: Border.all(
                    color:
                        active ? color.withOpacity(0.45) : Colors.transparent,
                    width: 1.5),
              ),
              child: Text(
                m,
                style: TextStyle(
                    fontSize: 13, // CHANGED: was 12
                    fontWeight: active ? FontWeight.w800 : FontWeight.w500,
...
// ═══════════════════════════════════════════════════════════════
//  SEARCH PAGE  — with curated defaults for All/Stocks/ETF tabs
// ═══════════════════════════════════════════════════════════════

class _SearchPage extends StatefulWidget {
  final meili.MeiliSearchClient meiliClient;
  const _SearchPage({required this.meiliClient});
...
class _SearchPageState extends State<_SearchPage> {
  ...
  static const _exchanges = ['NSE', 'BSE', 'NFO', 'MCX', 'CDS'];
  ...
  void initState() {
    ...
    // Fetch default data immediately based on initial segment
    _search('');
  }
  ...
  Future<void> _search(String q) async {
    ...
    try {
      ...
      // Meilisearch handles empty queries naturally.
      final res = await widget.meiliClient.index('stocks_zerodha').search(
          q.trim(),
          meili.SearchQuery(
            ...
            limit: q.trim().isEmpty ? 20 : 15, // Load a bit more for browsing
          ));

      final hits = res.hits.cast<Map<String, dynamic>>();
      final ql = q.trim().toLowerCase();

      // Only apply custom sorting if the user actually typed a query
      if (ql.isNotEmpty) {
        hits.sort((a, b) {
                  ...
                  child: Row(children: [
                    ...
                    if (_searching && _ctrl.text.isNotEmpty)
                      ...
                      GestureDetector(
                        onTap: () {
                          _ctrl.clear();
                          _search(''); // Refetch defaults on clear
                        },
                        child: Padding(
                            padding: const EdgeInsets.all(12),
                        ...
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(6),
                              border: Border.all(
                                  color: active ? t.div : Colors.transparent),
                            ),
                            child: Text(e,
                                style: TextStyle(
                                    fontSize: 11,
...
// ─────────────────────────────────────────────────────────────
//  Search result tile
//  CHANGED: removed redundant right-side strike/expiry panel.
//  For options/futures: expiry + strike shown inline in subtitle.
//  For stocks/ETFs: company name shown in subtitle. No right column.
// ─────────────────────────────────────────────────────────────
...
class _SearchResultTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Build subtitle inline — no separate right panel
...
class _OptionChainPageState extends State<_OptionChainPage> {
  ...
  Future<void> _onChanged(String q) async {
    ...
    try {
      final res = await widget.meiliClient.index('stocks_zerodha').search(
            ...
            meili.SearchQuery(
              filter: 'type = "option"',
              attributesToRetrieve: ['symbol', 'tradingsymbol'],
              limit: 50,
            ),
          );
      final hits = res.hits.cast<Map<String, dynamic>>();
  ...
  void _selectSuggestion(String underlying) {
    _ctrl.text = underlying;
    _focus.unfocus();
    setState(() {
      _suggestions = [];
      _showSuggestions = false;
...
class _ChainRow extends StatelessWidget {
  final _T t;
  final double strike;
  final Map<String, dynamic>? ceDoc;
  final Map<String, dynamic>? peDoc;
  final void Function(Map<String, dynamic>) onSelect;
  const _ChainRow(
      {required this.t,
