// Models
// Updated HoldingModel with new fields from API response
class HoldingModel {
  ...
  final double pnl; // This comes from API - keep it
  ...
  // Helper methods to get different quantity types
  ...
  // Calculate values using Zerodha's method
  double get investmentValue => averagePrice * (quantity + t1Quantity);
  double get currentValue => lastPrice * (quantity + t1Quantity);

  // Use the API's pnl value, but calculate percentage correctly
  double get pnlPercentage =>
      investmentValue > 0 ? (pnl / investmentValue) * 100 : 0;

  // Additional helper methods
  double get totalValue => currentValue; // Alias for clarity
  bool get isProfit => pnl >= 0;

  @override
  String toString() {
    return 'HoldingModel(symbol: $tradingSymbol, qty: $quantity, avg: $averagePrice, ltp: $lastPrice, pnl: $pnl)';
  }
}
...
class KiteMeta {
  ...
  factory KiteMeta.fromJson(Map<String, dynamic> json) {
    return KiteMeta(
      dematConsent: json['demat_consent'],
    );
  }
}
