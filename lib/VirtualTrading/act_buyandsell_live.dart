...
class BuyandSellPageLive extends StatefulWidget {
  final String? stockname;
  final String? segment;
  final bool tosell;

  const BuyandSellPageLive(this.stockname, this.segment, this.tosell,
      {Key? key})
      : super(key: key);

  @override
  _BuyandSellPageLiveState createState() => _BuyandSellPageLiveState();
}

class _BuyandSellPageLiveState extends State<BuyandSellPageLive> {
      ...
      if (_orderType == 'BUY') {
        if (_hasShortPosition) {
          ...
          if (totalQty <= _shortPositionQuantity) {
            // Covering entire order from short position
            _marginRequired = 0.0;
            // Add back the margin + profit/loss
            final shortValue = _shortPositionAvgPrice * totalQty;
            final buyValue = price * totalQty;
            final profitLoss = shortValue - buyValue;
            _projectedBalance = _originalAvailableBalance + profitLoss;
          } else {
            // Partially covering short, rest is new buy
        ...
        Container(
          ...
          child: TextFormField(
            ...
            validator: (value) {
              ...
              // No restriction on sell quantity for shorting
              // The validation for buyback is already handled by balance check
              return null;
            },
          ),
        ),
  ...
  Widget _buildMarginInfo(bool isDark) {
    ...
    if (_orderType == 'BUY' &&
      ...
      label = 'Buyback Value';
        ...
        _hasShortPosition &&
        totalQty > _shortPositionQuantity) {
      label = 'Net Margin Required';
    } else if (_orderType == 'BUY') {
      label = 'Margin Required';
    } else {
      label = 'Short Sell Value';
    }
